...
package ac.robinson.mediaphone_gtg.provider;
...
public class NarrativeItem implements BaseColumns {
	...
	public static final Uri NARRATIVE_CONTENT_URI = Uri.parse(MediaPhoneProvider.URI_PREFIX + MediaPhoneProvider
			.URI_AUTHORITY + MediaPhoneProvider.URI_SEPARATOR + MediaPhoneProvider.NARRATIVES_LOCATION);

	public static final Uri TEMPLATE_CONTENT_URI = Uri.parse(MediaPhoneProvider.URI_PREFIX + MediaPhoneProvider
			.URI_AUTHORITY + MediaPhoneProvider.URI_SEPARATOR + MediaPhoneProvider.TEMPLATES_LOCATION);
	...
	public static final String[] PROJECTION_NEXT_EXTERNAL_ID = new String[]{"MAX(" + NarrativeItem.SEQUENCE_ID + ") as" +
			" " + MAX_ID};
	...
	public ArrayList<FrameMediaContainer> getContentList(ContentResolver contentResolver) {
		...
		for (FrameMediaContainer container : exportedContent) {
			...
			for (int i = 0, n = container.mAudioPaths.size(); i < n; i++) {
				...
				if (audioCount != null) {
					container.updateFrameMaxDuration((int) Math.ceil(container.mAudioDurations.get(i) / (float)
							audioCount));
					longAudioFound = true;
				}
			}
	 ...
	 * <p/>
	 * Note: start time could easily be calculated from the narrative descriptor's mTimeToFrameMap,
	 * but as we're looping
	...
	public ArrayList<PlaybackMediaHolder> getPlaybackContent(ContentResolver contentResolver, String startingFrame,
		...
		for (FrameItem frame : narrativeFrames) {
			...
			// first we need to deal with durations; specifically audio - it's far more complex to play than
			// images/text
				...
				if (mediaType == MediaPhoneProvider.TYPE_AUDIO) {
					...
					if (media.getSpanFrames()) {
						...
						if (frameId.equals(media.getParentId())) {
							...
							// note: if changing behaviour, be sure to account for the similar version in
							// getContentList
							frameDuration = Math.max((int) Math.ceil(mediaDuration / (float) longRunningAudioCounts
									.get(mediaPath)), frameDuration);
						}
					} else {
					...
					// note that calculated text durations are stored as negative numbers so we can keep track of
					// what's
			...
			// note that we adjust image timings so that their crossfade ends when the next image starts - this
			// requires
			...
			// TODO: spread the crossfade over the beginning and the end of the image so that timings are more
			// accurate?
			...
			for (MediaItem media : frameComponents) {
				final int mediaType = media.getType();
				if (mediaType == MediaPhoneProvider.TYPE_IMAGE_FRONT || mediaType == MediaPhoneProvider
						.TYPE_IMAGE_BACK || mediaType == MediaPhoneProvider.TYPE_VIDEO) {

					// check whether this is a duplicate of the previous item - if so, just extend that item's duration
					if (media.getSpanFrames() && previousFrameImage != null && media.getInternalId().equals
							(previousFrameImage.mMediaItemId)) {
						...
						frameImage = new PlaybackMediaHolder(previousFrameImage.mParentFrameId,
								...
								lastFrameAdjustments ? -1 : (imageAdjustment ? narrativeDescriptor
										.mNarrativeImageAdjustment : 0));
						narrativeContent.set(replacementPosition, frameImage);
						previousFrameImage = frameImage; // we've replaced the old item

					} else {

						frameImage = new PlaybackMediaHolder(frameId, media.getInternalId(),
								media.getFile().getAbsolutePath(), mediaType, narrativeTime, mediaEndTime,
								imageAdjustment ? narrativeDescriptor.mNarrativeImageAdjustment : 0,
								lastFrameAdjustments ? -1 : (imageAdjustment ? narrativeDescriptor
										.mNarrativeImageAdjustment : 0));
			...
			for (MediaItem media : frameComponents) {
				if (media.getType() == MediaPhoneProvider.TYPE_TEXT) {
					...
					// check whether this is a duplicate of the previous item - if so, just extend that item's duration
					if (media.getSpanFrames() && previousFrameText != null && media.getInternalId().equals
							(previousFrameText.mMediaItemId)) {
						...
						frameText = new PlaybackMediaHolder(frameId, media.getInternalId(),
								media.getFile().getAbsolutePath(), MediaPhoneProvider.TYPE_TEXT, narrativeTime,
								mediaEndTime, 0, lastFrameAdjustments ? -1 : 0);
					...
					if (imageAdjustment && frameImage == null && previousFrameImage != null) {
						...
						previousFrameImage = new PlaybackMediaHolder(previousFrameImage.mParentFrameId,
								...
								previousFrameImage.getEndTime(false), narrativeDescriptor.mNarrativeImageAdjustment,
								0);
	...
	public String toString() {
		return this.getClass().getName() + "[" + mInternalId + "," + mCreationDate + "," + mSequenceId + "," +
				"" + mDeleted + "]";
	}
}
