...
public class FramesManager {
	 ...
	 * @param frameIds
	 */
	public static void reloadFrameIcons(Resources resources, ContentResolver contentResolver,
	                                    ArrayList<String> frameIds) {
	...
	public static FrameItem addFrameAndPreloadIcon(Resources resources, ContentResolver contentResolver,
	                                               FrameItem frame) {
	...
	public static ArrayList<String> findFrameIdsByParentId(ContentResolver contentResolver, String parentId) {
		...
		try {
			c = contentResolver.query(FrameItem.CONTENT_URI, FrameItem.PROJECTION_INTERNAL_ID,
					mFrameParentIdSelection, arguments, FrameItem.DEFAULT_SORT_ORDER);
			if (c.getCount() > 0) {
	...
	/**
	 * Returns a list of the frame ids following the given frame id. If includePrevious is set then the frame before
	 * the
