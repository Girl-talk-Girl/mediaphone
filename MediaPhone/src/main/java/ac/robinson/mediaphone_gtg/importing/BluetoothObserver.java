...
package ac.robinson.mediaphone_gtg.importing;
...
public class BluetoothObserver extends FileObserver {
	...
	private Map<String, Map<String, Boolean>> mSMILContents = Collections.synchronizedMap(new HashMap<String,
			Map<String, Boolean>>());
	...
	private boolean checkAndSendSMILContents(String smilParent, Map<String, Boolean> smilContents) {
		...
		if (allContentsComplete) {
			...
			if (MediaPhone.DEBUG) Log.d(DebugUtilities.getLogTag(this), "Sending SMIL");
			return true;
		}

		if (MediaPhone.DEBUG) Log.d(DebugUtilities.getLogTag(this), "SMIL not yet complete - waiting");
		return false;
	}
		...
		switch (event) {
			case CLOSE_WRITE:
				...
				if (IOUtilities.fileExtensionIs(fileAbsolutePath, MediaUtilities.HTML_FILE_EXTENSION)) {
					...
					// need to deal with some devices automatically deleting anything with a .smil extension - .sync
					// .jpg
					...
					if (!mSMILContents.containsKey(fileAbsolutePath)) {
						...
						Map<String, Boolean> smilContents = Collections.synchronizedMap(new HashMap<String,
								Boolean>());
						...
						ArrayList<String> smilUnparsedContents = SMILUtilities.getSimpleSMILFileList(receivedFile,
								true);
						...
						if (smilUnparsedContents != null) {
							for (String mediaFile : smilUnparsedContents) {
								...
								if (mIgnoredFiles.contains(smilMediaPath)) {
									...
									if (MediaPhone.DEBUG)
										Log.d(DebugUtilities.getLogTag(this), "SMIL component found (previously " +
												"recorded): " + smilMediaPath);
						...
						if (MediaPhone.DEBUG)
							Log.d(DebugUtilities.getLogTag(this), "SMIL already parsed - ignoring: " + receivedFile
									.getName());
					}
				}
			...
			case OPEN:
			default:
				if (MediaPhone.DEBUG) Log.d(DebugUtilities.getLogTag(this), "Other event: " + event);
				break;
		}
	}
