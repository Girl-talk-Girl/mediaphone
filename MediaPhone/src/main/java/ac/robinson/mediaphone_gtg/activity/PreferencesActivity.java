...
public class PreferencesActivity extends PreferenceActivity implements Preference.OnPreferenceChangeListener {
	 ...
	 * options like audio bit rate will be hidden on the oldest devices and those that don't support M4A,
	 * and the option
	...
	private void setupPreferences() {
		...
		if (DebugUtilities.supportsAMRAudioRecordingOnly()) {
			PreferenceCategory editingCategory = (PreferenceCategory) preferenceScreen.findPreference(getString(R
					.string.key_editing_category));
		...
		Preference installHelperPreference = preferenceScreen.findPreference(getString(R.string
				.key_install_helper_narrative));
		if (NarrativesManager.findNarrativeByInternalId(getContentResolver(), NarrativeItem.HELPER_NARRATIVE_ID) ==
				null) {
